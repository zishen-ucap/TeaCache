...
delta_output_list = []
delta_e0_list = []
sum_delta_input_list = [0]*28
sum_delta_output_list = [0]*28
sum_delta_e0_list = [0]*28
...
def teacache_forward(
    ...
    if self.enable_teacache:
        ...
        if self.cnt%2==0: # even -> conditon
            self.is_even = True
            if self.cnt < 10 or self.cnt == self.num_steps:
                    should_calc_even = True
                    self.accumulated_rel_l1_distance_even = 0
            else: 
                coefficients = [-3.03318725e+05, 4.90537029e+04, -2.65530556e+03, 5.87365115e+01, -3.15583525e-01]
                rescale_func = np.poly1d(coefficients)
                self.accumulated_rel_l1_distance_even += rescale_func(((e0-self.previous_e0_even).abs().mean() / self.previous_e0_even.abs().mean()).cpu().item())
                if self.accumulated_rel_l1_distance_even < self.rel_l1_thresh:
                    should_calc_even = False
                else:
                    should_calc_even = True
                    self.accumulated_rel_l1_distance_even = 0
            self.previous_e0_even = e0.clone()
            ...
            if self.cnt < 10 or self.cnt == self.num_steps:
                ...
                coefficients = [-3.03318725e+05, 4.90537029e+04, -2.65530556e+03, 5.87365115e+01, -3.15583525e-01]
                rescale_func = np.poly1d(coefficients)
                self.accumulated_rel_l1_distance_odd += rescale_func(((e0-self.previous_e0_odd).abs().mean() / self.previous_e0_odd.abs().mean()).cpu().item())
                if self.accumulated_rel_l1_distance_odd < self.rel_l1_thresh:
                    should_calc_odd = False
                else:
                    should_calc_odd = True
                    self.accumulated_rel_l1_distance_odd = 0
            self.previous_e0_odd = e0.clone()

    if self.enable_teacache: 
        if self.is_even:
            if not should_calc_even:
                # print(f"跳过第{self.cnt}步------------------------------------------------")
